...
/**
 *
 * @param {{width: 'md' | 'full', isOpen: boolean, onClose: Function}} props
 * @returns
 */
...
export function UiModal({
  ...
  const handleCloseClick = (e) => {
    const inModal = e.target.closest("[data-modal=modal]");
    if (inModal) {
      return;
    }
    onClose();
  ...
  const modal = (
    ...
    >
      ...
      >
        <button
          className="w-8 h-8 flex items-center justify-center rounded transition-colors bg-white/10 hover:bg-white/40 absolute top-0 left-[calc(100%+12px)]"
          onClick={onClose}
        >
